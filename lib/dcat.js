...
async function getDistributions(resource, {shareId, shareToken}) {
  ...
  const distributions = resource.links
    .filter(l => l.kind === 'data' && l.actions.includes('download'))
    .map(l => ({
      '@type': 'dcat:Distribution',
      identifier: l._id,
      title: l.title,
      format: getUrlExtension(l.url),
...
async function convertToDataset(resource, {shareId, shareToken}) {
  ...
  if (!dataset.description) {
    errors.push({level: 'W', message: 'Aucune description enrichie n’a pu être construite'})
  }

  if (dataset.distribution.length > 0) {
    errors.push({level: 'E', message: 'Aucun ressource téléchargeable n’a pu être construite'})
  }

  const isValid = !errors.some(e => e.level === 'E')

  return {dataset, isValid, errors}
}
...
function transformIntoDebug({shareId, shareToken}) {
  return pumpify.obj(
    createTransformStream({shareId, shareToken}),
    new Transform({
...
module.exports = {convertToDataset, transformIntoDcat, transformIntoDebug}
