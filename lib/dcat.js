...
async function getServices(resource){
  ...
  resourceServiceLayers.forEach((service) => {//Check if every service is cached
    if (!servicesCache.has(service._id)){
      serviceLayersToQuery.push(service)
    }
  })
    ...
    servicesInfo.servicelayers.forEach((s) => {//Add them to cache
      if (s.DCATqueryURL){
        servicesCache.set(s.serviceId, {
          ...
          title : s.layerTitle,
          downloadURL: { 'geojson' : s.DCATqueryURL}
        })
      }
      else {
        servicesCache.set(s.serviceId, {
  ...
  resourceServiceLayers.forEach((service) => {// Get every service in cache
    ...
    if (serviceValue){
      if (serviceValue.downloadURL){
        services.push({
          '@type': 'dcat:Distribution',
          identifier: serviceValue.identifier,
          title: serviceValue.title,
          format: 'geojson',
          downloadURL: serviceValue.downloadURL['geojson'],
