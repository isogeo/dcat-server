...
const { cp } = require('fs')
const servicesCache = new NodeCache({ stdTTL: 1000000, checkperiod: 500000 })
...
async function getServices(resource) {
  ...
  let serviceLayersToQuery = resourceServiceLayers.filter((service) => !servicesCache.has(service._id))

  if (serviceLayersToQuery.length > 0) {//If some services are not cached
    const servicesInfo = await got.post(GDP_API_URL + '/api/servicesParser/getAvailableServices', {
      json: ({ jsonData: { serviceLayers: serviceLayersToQuery } })
    }).json()

    servicesInfo.servicelayers.forEach((s) => {//Add them to cache
      servicesCache.set(s.serviceId, {
        identifier: s.serviceId,
        title: s.layerTitle,
        downloadURL: s.DCATqueryURLs
      }, s.DCATqueryURLs ? 1000000 : 500000)
    })
  }

  resourceServiceLayers.forEach((service) => {// Get every service in cache
    let serviceValue = servicesCache.get(service._id)
    SERVICE_OUPUTFORMAT.forEach((format) => {
      if (serviceValue && serviceValue.downloadURL && serviceValue.downloadURL[format]) {
        services.push({
          '@type': 'dcat:Distribution',
          identifier: serviceValue.identifier,
          title: serviceValue.title,
          format: format,
          downloadURL: serviceValue.downloadURL[format],
          issued: resource.modified
        })
      }
    })
  })
...
async function getDistributions(resource, { shareId, shareToken }) {
  let services = await getServices(resource)
