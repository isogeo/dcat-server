...
async function getUrlExtension(url) {
  ...
  if (url.startsWith(process.env.DCAT_SERVER_URL)) {
    try {
      ...
      if ((result.headers['content-type'].includes('json') || result.headers['content-type'].includes('octet-stream')) && result.headers['content-disposition'] && correctExtensions.length == 0) {
        const header = result.headers['content-disposition']
        const parts = header.split(';')
        filename = parts[1].split('=')[1].slice(0, -1)
      }
      else {
