...
async function getResourcesOffset(apiUrl, accessToken, shareId, offset) {
  const response = await got(`${apiUrl}/resources/search`, {
    searchParams: {
      ...
      _offset: offset,
      _include: 'links,serviceLayers,conditions,feature-attributes'
    },
    headers: {authorization: `Bearer ${accessToken}`},
    responseType: 'json'
  })
...
function getDownloadStream(apiUrl, accessToken, resourceId, linkId) {
  return got.stream(`${apiUrl}/resources/${resourceId}/links/${linkId}.bin`, {
    headers: {authorization: `Bearer ${accessToken}`}
  })
}
...
class Client {
  ...
  async getDownloadStream(resourceId, linkId) {
    const token = await this.getFreshToken()
    return getDownloadStream(this.apiUrl, token, resourceId, linkId)
  }
}

module.exports = {Client, getToken, getShare, getResourcesOffset, getResourcesStream}
